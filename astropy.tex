...
% TODO: references to the Astrophysics Source Code Library should include a mention of the ASCL
...
significantly enhanced in behavior and implementation\footnote{with the intent that the \texttt{pynbody} will eventually become interoperable with \texttt{astropy.units}}. This sub-package can be
...
coordinate systems, with only a class definition and two transformation functions.
...
was developed for over a decade, it has over time been made to work with an
...
\footnote{\url{http://www.atnf.csiro.au/people/mcalabre/WCS/wcslib/index.html}}. Since all of the FITS header parsing is done
...
\begin{itemize}
\item Improving inter-operability between packages, which includes for example seamlessly integrating the \texttt{astropy.units} framework across all sub-packages
...
\begin{itemize}
\item More Virtual Observatory tools, such as support for the Simple Image Access Protocol (SIAP)
