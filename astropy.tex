...
All coordinate systems are represented as classes, so internally, the 
transformation infrastructure keeps track of a graph of coordinate classes and
...
sequence of transformations transparently.  In practice, this means
...
transformation graph.  Once just one transform pair is implemented,
\texttt{astropy.coordinates} will be able to transform from that coordinate 
system to any other that is in the graph.  This greatly simplifies
implementation of the system, and mirrors the way most coordinate systems are
...
systems that are built-in, aside for conversions from celestial coordinates to
...
However, it requires an observer location, with additional bookeeping
necessary.  Hence, implementing such transformation has been delayed to a
...
units or as redshifts, and \texttt{astropy.coordinates} will transparently
make use of the cosmological calculations in \texttt{astropy.cosmology} when 
conversion from redshift to physical distances are needed. 
...
line-of-sight distances are provided,  direct and convinient calculation of
3-dimensional distances are enabled via the \texttt{separation3d} method of 
coordinate objects.
