...
The \texttt{astropy.units} package provides support for physical units. It
...
\fbox{\includegraphics[scale=0.82,clip]{examples/code_units.pdf}}
\end{figure}
...
strictly comparable. This unit package supports defining such relationships,
and it is one of the areas that distinguish itself from most other units
...
by supplying one or more input time values as well as the \textit{time format}
and \textit{time scale} of those values. The input time(s) can either be a
single scalar like \verb|"2010-01-01 00:00:00"| or \verb|2455348.5| or a
sequence of such values; the format specifies how to interpret the input
...
feel at home with this API. Figure~\ref{code:fits} shows and example of how to
open an existing FITS file and display its primary header.

\begin{figure}
\caption{Simple FITS file reading (TODO: needs expanding).\label{code:fits}}
