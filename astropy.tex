...
such as \texttt{kapteyn} \citep{kapteyn}, \texttt{pyast} \citep{pyast}, \texttt{pyephem} \citep{pyephem}, \texttt{palpy} \citep{palpy}, and
\texttt{astropysics} \citep{astropysics}, that this package aims to supplant.

%TODO: Comment from @mservillat: End of paragraph 1: what are the "lessons learned" exactly? Is it what is described in the next paragraph? then a clear link is needed (add "For example", "In particular"...). 
...
% TODO: Comment from Tim Jenness: We should state whether we support the modern nutation and precession models in general (I don't think we do but ERFA/SOFA does).
...
%TODO: Comment from @mservillat: naive question, but is there any link (or future link) with the coordinates subpackage? I think some people will have the question in mind at the end of this section.
...
%TODO: Comment from @mservillat: there is nothing about the license for Astropy (just a word in the intro about some affiliated packages not being compatible with the Astropy license). Shouldn't we say a word about the license?
