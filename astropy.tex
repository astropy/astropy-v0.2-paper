...
Adrian M. Price-Whelan\inst{\ref{inst:columbia}}  % confirmed
  \and
Wolfgang E. Kerzendorf\inst{\ref{inst:toronto}}  % confirmed
  \and
...
Henry Ferguson\inst{\ref{inst:stsci}}  % confirmed
  \and
Fr\'ed\'eric Grollier\inst{\ref{inst:freelance}}  % confirmed
  \and
...
Marseille, France
  ...
  \label{inst:saao}
    \and
  Joint Astronomy Centre, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA
  \label{inst:jac}
    \and
  Department of Astronomy, Cornell University, Ithaca, NY 14853, USA
  \label{inst:cornell}
    \and
  Gemini Observatory, 670 N.\ A`oh\=ok\=u Place, Hilo, Hawaii 96720, USA
  \label{inst:gemini_n}
    \and
...
and to date, over 200 individuals are signed up to the development mailing list for
...
monochromatic flux densities, which allows users to convert between
$F_\nu$ and $F_\lambda$, and users can easily implement their own
equivalencies.
...
physical constants and user-specified quantities.
...
We note that the \texttt{Time} class includes support for leap seconds
...
query the online SIMBAD\footnote{\url{http://simbad.u-strasbg.fr}} database to
...
The \texttt{astropy.coordinates} sub-package was designed such that it should be easy for a user to add new
coordinate systems.
...
scientific task (Price-Whelan \& Johnston 2013, in prep).
...
read and written, and once in memory can be easily modified. This
...
standard. This includes support for deprecated formats such as
...
and greatly reducing the memory footprint. VOTable files compressed
...
Development on the Astropy package is very active, and in addition to
some of the incremental improvements to existing sub-packages described in the
text, we are focusing on implementing major new functionality in
...
\item Implementing a Virtual Observatory cone search tool \citep{vo-cone-search}
...
\item Support for the Simple Image Access Protocol (SIAP) \citep{vo-siap}
\item Support for the Table Access Protocol (TAP) \citep{vo-tap} is under consideration
\end{itemize}
...
We thank the referee, Igor Chiligarian, for suggestions that helped improve this paper.
...
The name resolution functionality shown in
