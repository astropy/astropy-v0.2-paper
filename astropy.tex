...
representing cosmologies, and functions for calculating commonly used
quantities that depend on a cosmological model. It also contains a
framework for less frequently used cosmologies that may be not be
flat, or have a time-varying pressure to density ratio, $w$, for dark
energy. The quantities that can be calculated are generally taken from
those described by \citet{Hogg99}. Some examples are the angular
diameter distance, comoving distance, critical density, distance
modulus, lookback time, luminosity distance, and Hubble parameter as a
...
dark matter, dark energy densities at $z=0$. One can then use methods
of this class to perform calculations using these parameters.
...
70$~km\,s$^{-1}$\,Mpc and $\Omega_{M,z=0} = 0.3$ and bind it to the
name \texttt{cosmo}, one would use \texttt{cosmo =
  FlatLambdaCDM(H0=70, Om0=0.3)}. Then the comoving volume (for
example) can be calculated at redshift $6.5$ using the
\texttt{comoving\_volume} method;
\texttt{cosmo.comoving\_volume(6.5)}. Further calculations can be
performed using the many methods of \texttt{cosmo}, such as
\texttt{cosmo.lookback\_time} and \texttt{cosmo.comoving\_distance}.
For users who may be more comfortable using a procedural coding style,
these methods are also available as functions that take a cosmology
class instance as a keyword argument.

In addition to the ability to define arbitrary cosmologies, the
sub-package provides several pre-defined cosmology instances
corresponding to commonly used cosmological parameter sets. Currently
parameters from the from the WMAP 5-year, 7-year and 9-year results
are included (\texttt{WMAP5}, \texttt{WMAP7} and
\texttt{WMAP9}). There are several cosmology classes corresponding to
non-flat cosmologies, or those with a varying dark energy pressure to
density ratio. More exotic cosmologies can be represented by
sub-classing one of these basic cosmology classes.
